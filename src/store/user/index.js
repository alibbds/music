import { reqCaptcha,reqLoginState,reqLogout,reqUserLogin,reqUserCollection, reqUserPlayList, reqPlayList, reqUserLevel, reqUserMessage } from "@/api"
...
const getters = {
    ...
    playlistCreateByUser(state){
        return state.playlist.filter((item)=>{
            return item.creator.userId === JSON.parse(window.localStorage.getItem('userInfo')).userId
        })
    },
