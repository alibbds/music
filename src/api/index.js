...
export const reqCaptcha = (phone) =>
  request({ url: `/captcha/sent?phone=${phone}`, method: "get" });
//验证码登录
export const reqUserLogin = ({ phone, captcha }) =>
  request({
    url: `/login/cellphone?phone=${phone}&captcha=${captcha}`,
    method: "get",
  });
//密码登录
export const reqUserLoginByPassword = ({ phone, password }) =>
  request({ url: `/login/cellphone?phone=${phone}&password=${password}` });
//获取登录状态
export const reqLoginState = () =>
  request({ url: `/login/status`, method: "get" });
//退出登录
...
//获取用户收藏等等
export const reqUserCollection = () =>
  request({ url: `/user/subcount`, method: "get" });
//首页轮播图数据
export const reqBanner = () =>
  request({ url: `/banner?type=0`, method: "get" });
//获取用户等级
export const reqUserLevel = () =>
  request({ url: `/user/level`, method: "get" });
//获取我的消息
export const reqUserMessage = () => request({ url: `/msg/notices?limit=3` });

/* 歌单api */
//获取热门歌单分类
export const reqHotCategory = () =>
  request({ url: `/playlist/hot`, method: "get" });
//获取歌单
export const reqHotList = ({ offset = 0, limit, cat = "", order = "hot" }) =>
  request({
...
//获取相似歌单
export const reqSimilarPlayList = (id) =>
  request({ url: `/simi/playlist?id=${id}`, method: "get" });
...
//获取音乐详情
export const reqSongDetails = (id) =>
  request({ url: `/song/detail?ids=${id}` });
//获取相似音乐
export const reqSongSimilars = (id) => request({ url: `/simi/song?id=${id}` });
//喜欢该音乐或取消喜欢
export const reqLikeSong = ({ id, like }) =>
  request({ url: `/like?id=${id}&like=${like}` });
...
//全部专辑
export const reqAllAlbum = ({ area, limit, offset }) =>
  request({
...
export const reqAllMv = ({
  ...
  request({
    url: `/mv/all?area=${area}&type=${type}&offset=${offset}&limit=${limit}&order=${order}`,
  });
//获取mv播放链接
export const reqMvLink = (id) =>
  request({ url: `/mv/url?id=${id}`, method: "get" });
//获取mv详情
export const reqMvDetail = (id) =>
  request({ url: `/mv/detail?mvid=${id}`, method: "get" });
//获取相似mv
...
/* 歌手 */
//获取歌手分类
export const reqArtistCategory = ({ type, area, initial, offset, limit }) =>
  request({
    url: `/artist/list?type=${type}&area=${area}&initial=${initial}&offset=${offset}&limit=${limit}`,
    method: "get",
  });
//获取热门歌手(用于首页展示)
export const reqHotArtist = (limit) =>
  request({ url: `/top/artists?offset=0&limit=${limit}`, method: "get" });
//获取歌手专辑
...
//歌手单曲
export const reqArtistSongs = (id) =>
...
/* 评论 */
//获取热门评论
export const reqHotComment = ({ id, type }) =>
  request({ url: `/comment/hot?id=${id}&type=${type}`, method: "get" });
//获取所有评论(有bug)
export const reqComment = ({ type, id, sortType, cursor, pageSize, pageNo }) =>
  request({
    url: `/comment/new?type=${type}&id=${id}&sortType=${sortType}&cursor=${cursor}&pageSize=${pageSize}&pageNo=${pageNo}`,
  });
//获取mv评论
export const reqMvComment = ({ id, limit, offset, before = "" }) =>
  request({
    url: `/comment/mv?id=${id}&limit=${limit}&offset=${offset}&before=${before}`,
    method: "get",
  });
//获取专辑评论
export const reqAlbumComment = ({ id, limit, offset, before = "" }) =>
  request({
    url: `/comment/album?id=${id}&limit=${limit}&offset=${offset}&before=${before}`,
    method: "get",
  });
//给资源发评论或删除评论(若要回复评论要穿commentId,若要回复动态则要传threadId)t:1发送t:2回复
export const reqSendOrDeleteComment = ({ t, type, id, content }) =>
  request({
    url: `/comment?t=${t}&type=${type}&id=${id}&content=${content}`,
    method: "get",
  });
//获取歌曲评论
export const reqSongComment = ({ offset, limit, id }) =>
  request({
    url: `/comment/music?id=${id}&limit=${limit}&offset=${offset}`,
    method: "get",
  });
//给评论点赞
export const reqLikeComment = ({ type, cid, t, id }) =>
  request({
    url: `/comment/like?type=${type}&cid=${cid}&t=${t}&id=${id}`,
    method: "get",
  });
...
export const reqHotSearch = () =>
  request({ url: "/search/hot", method: "get" });
//获取搜索详情
export const reqSearchDetail = ({ keywords, limit, offset, type }) =>
  request({
